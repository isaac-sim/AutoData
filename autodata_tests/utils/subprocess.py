...
_SUBPROCESS_TIMEOUT_SEC = int(os.environ.get("AUTODATA_SUBPROCESS_TIMEOUT", "1200"))
...
def run_subprocess(cmd: list[str], env: dict | None = None, timeout_sec: int | None = None) -> None:
    ...
    Args:
        ...
        timeout_sec: Wall-clock timeout in seconds. Defaults to
            ``_SUBPROCESS_TIMEOUT_SEC`` (env ``AUTODATA_SUBPROCESS_TIMEOUT``, fallback 1200).
    ...
    try:
        ...
        sys.stderr.write(f"\n[autodata] Subprocess timed out after {timeout_sec}s\n")
        sys.stderr.flush()
        raise
