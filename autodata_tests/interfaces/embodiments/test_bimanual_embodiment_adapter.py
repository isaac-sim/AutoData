...
"""Unit tests for :mod:`autodata_interfaces.embodiments.bimanual_embodiment_adapter`."""
...
from autodata_interfaces.embodiments import AbsolutePoseWholeBodyBimanualAdapter, BimanualEefConfig, PoseObsKeys
...
def _bound(embodiment_adapter: AbsolutePoseWholeBodyBimanualAdapter, left_pos, right_pos):
    obs = {
        "policy": {
    ...
    embodiment_adapter.bind_env(MockEnv(obs_buf=obs))
...
def test_action_dim_no_passthrough_channels():
    # With no passthrough channels, action_dim is the hand-joints end.
    assert _embodiment_adapter().action_dim == 18


def test_action_dim_with_passthrough_channels():
    assert _embodiment_adapter({"body": (18, 22)}).action_dim == 22


def test_action_dim_multiple_passthrough_channels_takes_max_end():
    embodiment_adapter = _embodiment_adapter({"body": (18, 22), "head": (22, 25)})
    assert embodiment_adapter.action_dim == 25
...
def test_post_init_rejects_passthrough_channel_named_like_eef():
    with pytest.raises(AssertionError, match="collides with an eef name"):
        _embodiment_adapter({"left": (18, 20)})
...
def test_post_init_rejects_passthrough_channel_overlapping_pose_region():
    with pytest.raises(AssertionError, match="overlaps action region"):
        _embodiment_adapter({"body": (10, 16)})  # overlaps the [0, 18) pose+hand region
...
def test_get_eef_names():
    assert _embodiment_adapter().get_eef_names() == ("left", "right")
...
def test_get_eef_poses_returns_both_arms():
    embodiment_adapter = _bound(
        _embodiment_adapter(), left_pos=torch.tensor([[1.0, 2.0, 3.0]]), right_pos=torch.tensor([[4.0, 5.0, 6.0]])
    )
    poses = embodiment_adapter.get_eef_poses()
...
def test_action_to_target_eef_pose_decodes_both_pose_blocks():
    ...
    out = embodiment_adapter.action_to_target_eef_pose(action)
...
def test_action_to_target_eef_pose_rejects_wrong_shape():
    with pytest.raises(AssertionError, match="action shape must be"):
        _embodiment_adapter().action_to_target_eef_pose(torch.zeros(18))  # 1-D
...
def test_target_eef_pose_to_action_packs_poses_and_interleaves_hands():
    ...
    action = embodiment_adapter.target_eef_pose_to_action({"left": left_target, "right": right_target}, passthrough)
...
def test_target_eef_pose_to_action_writes_extra_channels():
    embodiment_adapter = _embodiment_adapter({"body": (18, 22)})
    ...
    action = embodiment_adapter.target_eef_pose_to_action({"left": target, "right": target}, passthrough)
...
def test_target_eef_pose_to_action_rejects_missing_keys():
    ...
    with pytest.raises(AssertionError, match="target_eef_pose_dict must have exactly"):
        embodiment_adapter.target_eef_pose_to_action(
            {"left": target}, {"left": torch.zeros(2), "right": torch.zeros(2)}
        )
    with pytest.raises(AssertionError, match="passthrough_action_dict must have exactly"):
        embodiment_adapter.target_eef_pose_to_action({"left": target, "right": target}, {"left": torch.zeros(2)})
...
def test_actions_to_passthrough_actions_rejects_wrong_last_dim():
    with pytest.raises(AssertionError, match="actions last dim must be"):
        _embodiment_adapter().actions_to_passthrough_actions(torch.zeros(2, 17))
...
def test_passthrough_round_trip():
    ...
    action = embodiment_adapter.target_eef_pose_to_action({"left": target, "right": target}, passthrough)
    recovered = embodiment_adapter.actions_to_passthrough_actions(action.unsqueeze(0))
...
def test_canonicalize_quat_flips_negative_w():
    ...
    left_quat_canon = _embodiment_adapter(canonicalize=True).target_eef_pose_to_action(
        {"left": target, "right": target}, passthrough
    )[3:7]
    left_quat_raw = _embodiment_adapter(canonicalize=False).target_eef_pose_to_action(
        {"left": target, "right": target}, passthrough
    )[3:7]
...
def test_action_noise_zero_is_noop():
    ...
    passthrough = {"left": torch.tensor([0.0, 0.0]), "right": torch.tensor([0.0, 0.0])}
    no_noise = embodiment_adapter.target_eef_pose_to_action(
        {"left": target, "right": target}, passthrough, action_noise_dict=None
    )
    zero_noise = embodiment_adapter.target_eef_pose_to_action(
...
def test_from_dict_full():
    ...
    embodiment_adapter = AbsolutePoseWholeBodyBimanualAdapter.from_dict(data)
    assert embodiment_adapter.name == "g1"
    assert embodiment_adapter.left.gripper_action_indices == (0, 1)
    assert embodiment_adapter.right.gripper_action_indices == (2, 3)
    assert embodiment_adapter.left_pose_slice == (0, 7)
    assert embodiment_adapter.hand_joints_slice == (14, 18)
    assert embodiment_adapter.canonicalize_quat is False
    assert embodiment_adapter.passthrough_channels == {"body": (18, 22)}
    assert embodiment_adapter.action_dim == 22
...
def test_from_dict_defaults():
    ...
    assert embodiment_adapter.canonicalize_quat is True
    assert embodiment_adapter.passthrough_channels == {}
