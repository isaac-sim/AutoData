...
axis-angle conversion. Quaternions are ``(w, x, y, z)``, axis-angle vectors
are ``axis * angle`` (magnitude is the rotation angle in radians), and
rotation matrices act on column vectors.
"""
...
def make_pose(pos: torch.Tensor, rot_mat: torch.Tensor) -> torch.Tensor:
    ...
    assert pos.shape[:-1] == rot_mat.shape[:-2], (
...
def matrix_from_quat(quat: torch.Tensor) -> torch.Tensor:
    ...
    Args:
        quat: Quaternion of shape (..., 4) in (w, x, y, z) order.
...
def quat_from_matrix(rot_mat: torch.Tensor) -> torch.Tensor:
    ...
    w_branch = torch.stack([0.25 * sw, (m21 - m12) / sw, (m02 - m20) / sw, (m10 - m01) / sw], dim=-1)
    x_branch = torch.stack([(m21 - m12) / sx, 0.25 * sx, (m01 + m10) / sx, (m02 + m20) / sx], dim=-1)
    y_branch = torch.stack([(m02 - m20) / sy, (m01 + m10) / sy, 0.25 * sy, (m12 + m21) / sy], dim=-1)
    z_branch = torch.stack([(m10 - m01) / sz, (m02 + m20) / sz, (m12 + m21) / sz, 0.25 * sz], dim=-1)
...
def quat_unique(quat: torch.Tensor) -> torch.Tensor:
    ...
    """
    assert quat.shape[-1] == 4, f"quat last dim must be 4, got {quat.shape[-1]}"
    sign = torch.where(quat[..., 0:1] < 0, -torch.ones_like(quat[..., 0:1]), torch.ones_like(quat[..., 0:1]))
    return quat * sign
