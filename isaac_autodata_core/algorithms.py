    ...
    def plan_subtask_trajectory(
        ...
        *,
        ...
        if not planning_success:
            print(
                f"[SkillGen] env={env_id} subtask={eef_state.current_subtask_index} "
                f"eef={eef_name!r} attach={expected_attached_object!r}  PLAN_FAIL",
                flush=True,
            )
            return None
        ...
        print(
            f"[SkillGen] env={env_id} subtask={eef_state.current_subtask_index} "
            f"eef={eef_name!r} attach={expected_attached_object!r}  PLAN_OK "
            f"mp={len(mp_waypoints)} skill={len(subtask_traj)}",
            flush=True,
        )
