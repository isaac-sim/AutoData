...
* **Adaptability** — because transitions are planned fresh for each attempt, the same annotated
...
the new object poses are from the source demonstration. SkillGen splits every subtask into
two phases instead:
...
3. **Plan.** The planner syncs its collision world to the live scene — attaching the held
   ...
   end-effector pose to the segment's start. If planning fails, the attempt is abandoned and
   counted as a failure.
4. **Execute.** The transit waypoints are executed, then the skill segment replays with
   per-step action noise for diversity.
5. **Record.** The task's success condition is checked every step; successful attempts are
   exported as demonstrations to the output dataset.
...
Annotation is manual: each episode replays in a Kit window, and you mark signals with
the keyboard.
...
four-subtask cube-stacking task that is **7 marks** per episode, in this order:
...
.. tip::
   ...
   stacking and stacking inside a bin — generate from the same annotated cube-stacking dataset.
...
Since the Rerun viewer is independent of the Kit window, ``--visualize_plan`` works
