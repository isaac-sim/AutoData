...
per-step ``action_noise`` adds diversity across trials. A trial succeeds if the task's
success condition holds at any step; only then is it exported (subject to the
...
* **Source selection scope** — by default all arms replay segments from the same source demo
...
.. list-table::
   ...
   * - Strategy
     - Behavior
   * - ``random``
     - Uniform random choice over source demos.
   * - ``nearest_neighbor_object``
     - Ranks demos by how close their reference-object pose is to the current one, then
       picks randomly among the ``nn_k`` nearest.
   * - ``nearest_neighbor_robot_distance``
     - Ranks demos by how close their (object-frame-transformed) source end-effector pose
...
keeps some randomness so the output does not collapse onto a single source demo. The shipped
